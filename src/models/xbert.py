...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and self.save_attention:
            ...
            """
            register_hook的作用: 即对attention_probs求导时，对attention_probs的导数进行操作
            并且register_hook的参数只能以函数的形式传过去
            """       
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i in range(start_layer, output_layer):
            # print('Encoder Layer',i,output_layer)
...
class BertEncoderExtended(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([mb.BertLayer(config) for _ in range(config.encode_hidden_layers - 1)])
